import { IGridCoordinate } from "../types/coordinate";
import { ItemType } from "../types/grid";
...
export class GridCoordinate extends Coordinate {
  ...
  public getCellRect(rowIndex: number, columnIndex: number) {
    const { size: height, offset: y } = this.getCellMetaData(
      rowIndex,
      ItemType.Row,
    );
    const { size: width, offset: x } = this.getCellMetaData(
      columnIndex,
      ItemType.Column,
    );
    return {
