...
import { AvatarSize } from "../constants/avatar";
import { getTextWidth, textDataCache } from "./get-text-width";
import { imageCache } from "./image-cache";
import WebOutlined from "../icons/web-outlined";
import { IHyperlinkSegment, ISegment, SegmentType } from "../types/field";
...
export class Drawer {
  ...
  public textEllipsis(props: ITextEllipsisProps) {
    ...
    let guessText = text.slice(0, guessSize);
    ...
    while (guessWidth <= maxWidth) {
      ...
      guessSize++;
      guessText = text.slice(0, guessSize);
  ...
  public wrapText(props: IWrapTextProps) {
    ...
    let rowCount = 0;
    let linkIndex = 0;
    const linkMap: { [key: number]: { endIndex: number; url: string } } = {};
    ...
    const isEllipsis = this.textEllipsis({
      text,
      maxWidth: maxWidth,
    }).isEllipsis;
    ...
    for (let n = 0; n < textLength; n++) {
      ...
      const singleTextWidth = isLineBreak
        ? 0
        : this.ctx.measureText(singleText).width;
      ...
      if ((isLineEnd || isLineBreak) && rowCount < maxRow) {
        ...
        resultData.push({
          ...
          text: isLimitRow
            ...
            : showText,
          linkUrl:
            curLinkData && curLinkData.endIndex >= n ? curLinkData.url : null,
        });
        showText = singleText;
        offsetX = 0;
        showTextWidth = singleTextWidth;
