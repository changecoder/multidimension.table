...
const name = "web_outlined";
const defaultColors = ["#D9D9D9"];
const width = "16";
const height = "16";
const viewBox = "0 0 16 16";

export const WebOutlined = defineComponent({
  name: "WebOutlined",
  props: {
    size: {
  ...
  setup(props) {
    const pathColors = computed(() => {
      let colors = [props.color].flat();
      if (props.currentColor) {
        colors = Array(defaultColors.length).fill("currentColor");
      }
      if (colors.length < defaultColors.length) {
        colors.splice(colors.length, 0, ...defaultColors.slice(colors.length));
      }
      return colors;
    });

    const svgFill = computed(() => pathColors.value[0]);
    ...
    return () =>
      h(
        ...
        [
          props.showTitle ? h("title", name) : null,
          h("path", {
            d: allPathData[0],
            fill: "inherit",
          }),
...
WebOutlined.toString = () => allPathData.join("\n");
